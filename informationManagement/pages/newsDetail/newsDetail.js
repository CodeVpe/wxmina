...
Page({
  ...
  payOrder: function (orderId) {
    ...
    function paySuccess() {
      ...
      app.turnToPage('/informationManagement/pages/newsDetail/newsDetail?detail=' + that.data.articleId + franchiseeParam, 1);
      wx.showToast({
        title: '打赏成功',
        icon: 'success',
      })
      ...
      success: function (res) {
        if (res.status == 0) {
          ...
          if (liked == 1) {
            ...
            newData['commentList[' + index + '].like_count'] = likeNum;
            newData['commentList[' + index + '].like_count_text'] = likeNum <= 0 ? '赞' : (likeNum > 10000 ? (Math.floor(likeNum / 10000) + '万') : likeNum);
