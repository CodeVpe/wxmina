...
Page({
    ...
    app.getOrderDetail({
      ...
      success: function(res) {
        var data = res.data[0],
          ...
          hasAdditionalInfo = false,
          start_date = form_data.appointment_order_info.appointment_start_date,
          end_date = form_data.appointment_order_info.appointment_end_date,
        ...
        form_data.remark = form_data.remark ? form_data.remark.replace(/\n|\\n/g, '\n') : form_data.remark;
        if (start_date && end_date){
          if (start_date.substring(0, 4) == end_date.substring(0, 4)) {
            form_data.appointment_order_info.appointment_start_date = start_date.substring(4, 6) + '-' + start_date.substring(6);
            form_data.appointment_order_info.appointment_end_date = end_date.substring(4, 6) + '-' + end_date.substring(6);
          } else {
            form_data.appointment_order_info.appointment_start_date = start_date.substring(0, 4) + '-' + start_date.substring(4, 6) + '-' + start_date.substring(6);
            form_data.appointment_order_info.appointment_end_date = end_date.substring(0, 4) + '-' + end_date.substring(4, 6) + '-' + end_date.substring(6);
          }
        }
        if (form_data.appointment_order_info.appointment_unit_type == '6'){
          form_data.timeDiff = Date.parse(new Date()) - Date.parse(new Date(form_data.appointment_time.split('~')[1])) > 0 ? true : false;
        }else{
          form_data.timeDiff = false;
        }
        that.setData({
          orderData: data,
          orderInfo: form_data,
  ...
  showAddAddress: function() {
    ...
    if (wx.chooseAddress) {
      ...
      var orderId = this.data.orderInfo.order_id;
      app.turnToPage('/eCommerce/pages/addAddress/addAddress?oid=' + orderId);
    }
  },
