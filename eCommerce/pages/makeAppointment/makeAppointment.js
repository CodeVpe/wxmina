...
Page({
  ...
  dataInitialName: function () {
    var unitType = this.data.unitType,
      isHotelService = this.data.isHotelService,
      serviceTimeUnit,
      servicePrice,
      unitTime;
    if (unitType != 6) {
      ...
      success: function (res) {
        ...
        if (initial) {
          ...
          if (serviceTimeUnit == '天') {
            ...
            canSelectTimeLong = response.can_select_time_long.sort(sortArr);
        ...
        for (var index in appointmentInfo) {
          var periodInterval = appointmentInfo[index].interval,
            ...
            endTime = endHours >= 24 ? '次日' + (endHours * 1 - 24) + ':' + endMin : '当日' + (endHours * 1) + ':' + endMin;
  ...
  setTime: function (appointmentTime, timeNum) {
    ...
    sec = totalSec > 59 ? totalSec - (Math.floor(totalSec / 60) * 60) : totalSec;
    sec = sec > 9 ? sec : '0' + sec;
    hours = totalSec > 59 ? Math.floor(totalSec / 60) + hours * 1 : hours;
    totalTime = hours + ':' + sec;
    return totalTime;
  },
  //预约日期
  ...
  bindStartDateChange: function (e) {
    var value = e.detail.value,
      endValueStamp = this.data.isHotelService ? new Date(value).getTime() + 86400000 : new Date(value).getTime(),
      endValue = this.timestampToTime(endValueStamp);
    this.setData({
      'appointmentInfo.appointmentStartDateRange': value,
      'appointmentInfo.appointmentEndDateRange': endValue
    });
    this.getAppointmentList({
  ...
  bindEndDateChange: function (e) {
    var value = e.detail.value,
      endValueStamp = new Date(value).getTime(),
      startValueStamp = new Date(this.data.appointmentInfo.appointmentStartDateRange).getTime();

    if (endValueStamp < startValueStamp || (endValueStamp <= startValueStamp && this.data.isHotelService)) {
      app.showModal({
        content: '结束日期有误，请重新选择'
      })
      return false;
    }
    this.setData({
  ...
  bindTimeChange: function (e) {
    this.setData({
     ...
     if (e.currentTarget.dataset.unit == 6) {
      return false;
    }
    this.serviceConditionChange();
  ...
  getShopParam: function () {
    ...
    if (this.data.serviceTimeUnit == '小时') {
      ...
      var startHours = this.data.selectedPeriod.split('-')[0].split(':')[0],
        ...
        endHoursLog = this.data.selectedPeriod.split('-')[1].split(':')[0].substring(0, 2),
        endHours = parseInt(this.data.selectedPeriod.split('-')[1].split(':')[0].substring(2)),
        endHours = endHoursLog == "次日" ? endHours + 24 : endHours,
      ...
      if (this.data.unitType == "6") {
        param.appointment_day = this.data.appointmentInfo.appointmentStartDateRange.split('-').join('')
      }
      param.appointment_unit_type = this.data.unitType;
